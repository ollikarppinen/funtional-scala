...
abstract class TweetSet {
    ...
    /**
     * This is a helper method for `filter` that propagates the accumulated tweet's.
     */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def mostRetweeted = {
        ...
        if (!left.isEmpty && leftMost.retweets > elem.retweets)
            if (!right.isEmpty && rightMost.retweets > leftMost.retweets) rightMost else leftMost
