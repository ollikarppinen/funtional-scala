...
class ListsSuite extends FunSuite {
  ...
  test("sum of negative numbers") {
    assert(sum(List(-2, -5)) === -7)
  }

  test("sum of positive and negative numbers") {
    assert(sum(List(-2, -5, 4, 8)) === 5)
  }
  ...
  test("max of negative numbers") {
    assert(max(List(-2, -5)) === -2)
  }

  test("max of positive and negative numbers") {
    assert(max(List(-2, -5, 4, 8)) === 8)
  }

  test("max throws exception with empty list") {
